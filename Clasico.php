        ...
        <section id="contact" class="contact mb-5">
            <div class="container" data-aos="fade-up">
                ...
                <div class="col-md-9" data-aos="fade-up">
                    <h3 class="category-title">Genero: Clasicos</h3>
		      <div>
    			<form class="d-flex" action="" method="GET">
        			<input class="form-control me-2" type="search" name="query" placeholder="Libro" aria-label="Search">
        			<button class="btn btn-outline-success" type="submit">Buscar</button>
    			</form>
		      </div>
		<!-- Aquï¿½ comienza el cï¿½digo PHP para mostrar los libros del gï¿½nero "Clasicos" -->
                    <?php
                    // Incluir el archivo de conexiï¿½n
                    include 'conexion.php';

		    // Obtener la consulta de búsqueda
		    $query = isset($_GET['query']) ? $_GET['query'] : '';

                    // Consulta SQL para obtener los datos de la tabla libros filtrados por la categorï¿½a Clasicos
                    $sql = "SELECT Titulo, Autor, Categoria, Sinopsis FROM Libros WHERE Categoria = 'Clasicos'";
                    $result = $conn->query($sql);
		
		    if (!empty($query)) {
    		    $sql .= " AND (Titulo LIKE '%" . $conn->real_escape_string($query) . "%' OR Autor LIKE '%" . $conn->real_escape_string($query) . "%')";
		    }
		    ...
		    $result = $conn->query($sql);
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        echo "No hay libros en la categorï¿½a Clasico";
                    }
